	...
	return {
		...
		async configResolved() {
			...
			const _SOURCES = sources.filter(($) => {
				const sub = path.join(
					_EXPORT_PATH,
					`/${$.id}.${$.name.split(" ").join("_").toLowerCase()}.json`
				);
			...
			await Promise.all(
				_SOURCES.map(async ($) => {
					console.log(
						`[Subtitles plugin] Creating ${$.id}.${$.name}.json...`
					);
					...
					try {
						...
						await fsPromises.writeFile(
							path.join(_EXPORT_PATH, `/${$.id}.${$.name}.json`),
							JSON.stringify(json)
						);
						...
						console.log(
							`[Subtitles plugin] ${$.name}.json file generated`
						);
					} catch (error) {
						console.error(
