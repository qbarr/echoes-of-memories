...
function orbitController(
	...
	function updatePosition() {
		...
		const prevPos = tempVec3a.copy(object.position);
		...
		// console.log(offset, getTargetPosition(target), offsetedTarget);
		...
		if (prevPos.distanceToSquared(object.position) > 0.000001) {
	...
	function setFPSMode(v) {
		...
		if (fps) {
			object.getWorldPosition(tempVec3b);
			const dir = object.localToWorld(tempVec3a.set(0, 0, 1))
				.sub(tempVec3b)
	...
	function onChange(callback) {
		...
		return () => moveCallbacks.delete(callback);
	}
	...
	return {
		...
		dolly,
		onChange,
		get offsetedTarget() { return offsetedTarget },
		...
		get sphericalTarget() { return sphericalTarget },
	};
}
