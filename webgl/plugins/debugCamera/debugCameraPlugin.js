...
export function debugCameraPlugin(webgl) {
	...
	function initGUI() {
		...
		btn = gui.copyButton = gui.addButton({ title: `Reset to Target` });
		btn.on('click', () => resetToTarget());
	...
	function copyCoords() {
		...
		const posStr = api.currentCamera.base.position.toArray().map(v => v.toFixed(5)).join(', ');
		const qtStr = api.currentCamera.base.quaternion.toArray().map(v => v.toFixed(6)).join(', ');
	...
	function refreshTargets() {
		...
		const options = [ ...api.scene.targets.keys() ]
			.reduce((p, c) => (p[ c ] = c, p), {});
