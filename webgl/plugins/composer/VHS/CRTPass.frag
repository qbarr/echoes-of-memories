precision highp float;

#include <common>
#include <props>

uniform sampler2D tMap;
uniform sampler2D tDepth;

varying vec2 vUv;

const float XRES = 54.0 * 5.0;
const float YRES = 33.0 * 5.0;

#define SCANFLICKER .23
#define INTERFERENCE1 .6
#define INTERFERENCE2 0.002

const vec2 fishEye = vec2(0.1, 0.24);
const float vignetteRounding = 130.0;
const float vignetteSmoothness = .8;

vec2 fisheye(vec2 uv) {
	uv *= vec2(1.0 + (uv.y * uv.y) * fishEye.x, 1.0 + (uv.x * uv.x) * fishEye.y);
	return uv * 1.02;
}

float vignette(vec2 uv) {
	uv *= 1.99;
	float amount = 1.0 - sqrt(pow(abs(uv.x), vignetteRounding) + pow(abs(uv.y), vignetteRounding));
	return smoothstep(0., vignetteSmoothness, amount);
}

float peak(float x, float xpos, float scale) {
	return clamp((1.0 - x) * scale * log(1.0 / abs(x - xpos)), 0.0, 1.0);
}

float random(vec2 co, float time) {
	return fract(sin(dot(co.xy, vec2(12.9898, 78.233)) + time) * 43758.5453);
}

float round(float x) {
	return floor(x + 0.5);
}

void main() {
	vec2 uv = vUv;
	vec2 res = uResolution.xy * uResolution.z;
	vec2 uv2 = gl_FragCoord.xy / res;
	// vec2 squareUv = uv;
	// squareUv -= 0.5; // <-0.5,0.5>
	// if (res.x > res.y)
	// 	squareUv.x *= res.x / res.y;
	// else
	// 	squareUv.y *= res.y / res.x;
	// squareUv += 0.5; // <-0,1>

	float scany = round(uv2.y * YRES);

	uv -= 0.5;
	uv = fisheye(uv);
	float vign = vignette(uv);
	uv += 0.5;

	vec4 tex = texture2D(tMap, uv);
	float mframe = (mod(tex, vec4(0.)).g + mod(tex, vec4(0.)).r + mod(tex, vec4(0.)).b) / 3.0;
	uv2.y += mframe * 1.0 / YRES * SCANFLICKER;

	// Interference
	float r = random(vec2(0.0, scany), uTime);
	if (r > 0.995)
		r *= 3.0;

	float ifx1 = INTERFERENCE1 * 2.0 / res.x * r;
	float ifx2 = INTERFERENCE2 * (r * peak(uv.y, 0.2, 0.2));
	uv.x += ifx1 + -ifx2;

	vec3 texel = texture2D(tMap, uv).xyz;

	float scanl = 0.5 + 0.5 * abs(sin(PI * uv2.y * YRES));
	scanl = mix(scanl, 1.0, 0.5);

	vec3 rgb = vign * texel * scanl;
	gl_FragColor = vec4(rgb, 1.0);
}
