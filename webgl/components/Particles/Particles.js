...
import { AdditiveBlending, BufferAttribute, BufferGeometry, Group, Object3D, Points, ShaderMaterial, Uniform, Vector2 } from 'three';
...
export class Particles extends BaseComponent {
	...
	update() {
		...
		const modelParticlesTexture = this.gpgpu.computation.getCurrentRenderTarget(this.gpgpu.variables.particles).texture
		const dustParticlesTexture = this.gpgpu.computation.getCurrentRenderTarget(this.gpgpu.variables.dustParticles).texture
