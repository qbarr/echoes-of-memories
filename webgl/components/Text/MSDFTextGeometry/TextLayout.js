...
const RETURN_ID = '\r'.charCodeAt(0);
const NEWLINE_ID = '\n'.charCodeAt(0);
...
export class TextLayout {
        ...
        lines.forEach((line, lineIndex) => {
            ...
            for (let i = start; i < end; i++) {
                ...
                if (glyph) {
                    ...
                    let tx = x;
                    if (align === ALIGN_TYPES.center) {
                        tx += (maxLineWidth - lineWidth) / 2;
                    } else if (align === ALIGN_TYPES.right) {
    ...
    getGlyph(font, id) {
        ...
        if (glyph) {
            return glyph;
        } else if (id === TAB_ID || id === RETURN_ID || id === NEWLINE_ID) {
...
function getAlignType(align) {
	...
	if (ALIGN_TYPES[align] !== undefined) return ALIGN_TYPES[align];

	switch (align) {
		case 'center': return ALIGN_TYPES.center;
		case 'right': return ALIGN_TYPES.right;
		case 'left':
		default:
			return ALIGN_TYPES.left;
	}
}
